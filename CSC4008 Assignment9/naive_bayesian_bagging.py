...
class Instance:
    def __init__(self, attr, iclass, iclassified=0):
        self.attr = attr
        self.iclass = iclass
...
def main():
    ...
    b = eval(input("Please input b for the iteration times: "))
    step = int(n / k)
    ...
    for i in range(k):
        ...
        for j in range(b):
            ...
            for instance in test:
                ...
                for attribute in attribute_name:
                    if attribute in attribute_discrete_name:
                        p_X1 *= (sta1[attribute][instance.attr[attribute]] + 1) / (
                                class1 + len(attribute_type[attribute]))
                        p_X2 *= (sta2[attribute][instance.attr[attribute]] + 1) / (
                                class2 + len(attribute_type[attribute]))
                    else:
